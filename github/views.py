...
def post_pr_comments(payload: GithubPRChanged, review_response: PRReviewResponse):
    ...
    try:
        ...
        print(f"🔍 Parsed diff info for {len(diff_info)} files: {list(diff_info.keys())}")
        
        # Debug: Print some line mappings
        for file_path, line_map in diff_info.items():
            if line_map:
                print(f"📊 {file_path}: lines {min(line_map.keys())}-{max(line_map.keys())} available")
            else:
        ...
        for issue in review_response.issues:
            ...
            if diff_position is None:
                print(f"⚠️ Line {line_num} in file {file_path} not found in diff, posting as general PR comment")
                # Fall back to posting as a general PR comment (issue comment)
                success = _post_general_pr_comment(payload, installation_token, comment_body, file_path, line_num)
                if success:
            ...
            print(f"📝 Posting PR comment to {file_path}:{line_num} (diff position: {diff_position})")
            ...
            if response.status_code == 201:
                ...
                print(f"🔄 Trying fallback to general PR comment for {file_path}:{line_num}")
                success = _post_general_pr_comment(payload, installation_token, comment_body, file_path, line_num)
                if success:
                    successful_comments += 1
...
def _get_diff_position(diff_info: Dict[str, Dict[int, int]], file_path: str, line_num: int) -> Optional[int]:
    """
...
def _post_general_pr_comment(payload: GithubPRChanged, installation_token: str, comment_body: str, file_path: str, line_num: int) -> bool:
    """
    ...
    try:
        ...
        enhanced_body = f"**File: `{file_path}` (around line {line_num})**\\n\\n{comment_body}"
